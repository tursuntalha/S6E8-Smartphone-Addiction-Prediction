...
# tek-seed referans (seed 42, mevcut best pipeline ile birebir karsilastirilabilir)
ref_oof = (rankdata(all_oof[('lgb', 42)]) + rankdata(all_oof[('xgb', 42)]) + rankdata(all_oof[('cat', 42)])) / 3
ref_auc = roc_auc_score(y, ref_oof)
print(f'\nTek-seed (42) 3-model blend OOF AUC: {ref_auc:.5f}  (referans: v5 AB blend 0.96843)')

# cok-seedli: her modelin 3 seed ortalamasini al, sonra 3 modeli rank-blend et
...
print(f'Cok-seedli (3 seed ort, model-bazli sonra blend) OOF AUC: {multiseed_auc:.5f}  (delta={multiseed_auc-ref_auc:+.5f})')

# alternatif: tum 9 (model,seed) ciftini dogrudan rank-blend et
