"""
Izole test A: K1 bundle (48 ozellik) + frekans (count) encoding.
tomasa2'nin ("S6E8: What Moved the Score", LB 0.97014) notebook'unda "target+frequency
encoding, TUM sutunlarda" tekniginin bizde eksik olan parcasi: biz zaten target-mean TE
yapiyoruz (gun-1'den beri), ama frekans (count) encoding hic eklenmedi. Her exact-deger
icin train+test birlesik sayimi (target kullanilmadigi icin leakage yok, transduktif,
guvenli) yeni bir 'freq_' sutunu olarak ekleniyor.
...
for c in all_cats + te_only_cats:
    ...
    # target-mean TE (10-fold OOF), mevcut yontem, degismedi
    ...
    # YENI: frekans (count) encoding, train+test birlesik sayim, target kullanilmiyor (leakage yok)
...
X = pd.concat([raw_tr, enc_tr.add_prefix('te_'), freq_tr.add_prefix('freq_')], axis=1).values
X_test = pd.concat([raw_te, enc_te.add_prefix('te_'), freq_te.add_prefix('freq_')], axis=1).values
print(f'Ozellik sayisi: {X.shape[1]} (K1 48 + {freq_tr.shape[1]} frekans)')
...
sub_path = f'{SUB}/2026-08-19/lgbm_xgb_cat_freqenc_2026-08-19.csv'
