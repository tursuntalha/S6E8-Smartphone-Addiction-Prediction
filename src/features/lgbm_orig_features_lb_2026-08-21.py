"""
ORIG-CDF feature testinin (lgbm_orig_features_2026-08-21.py, LGB-solo CV delta=+0.00002
gurultu) LB dogrulamasi + feature importance analizi. Kullanici, CV sinyali zayif olsa da
(gecmiste Delta=+0.00014 OOF -> +0.00018 LB seklinde CV'nin kucumsedigi bir ornegimiz
oldugu icin) LB'de test etmek istedi. Karsilastirilabilir olmasi icin tek-model degil,
PRODUCTION ile AYNI 3-model (LightGBM+XGBoost+CatBoost) rank-blend kullanildi - referans:
K1+A+B+D 3-model blend LB=0.96985 (ORIG'siz). Feature importance (LightGBM gain) ayrica
raporlaniyor - ORIG feature'larin siralamada nereye dustugune odaklanarak.
"""
import pandas as pd
import numpy as np
import json
...
print(f'Toplam ozellik sayisi: {X_orig.shape[1]}  ({time.time()-t0:.0f}s)')

skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)
fold_idx = list(skf.split(X_orig, y))
...
Xv = X_orig.values
Xtestv = X_test_orig.values

oof_lgb = np.zeros(len(Xv)); pred_lgb = np.zeros(len(Xtestv))
...
for fold, (tr, va) in enumerate(fold_idx):
    ...
    print(f'  LightGBM fold {fold+1}/{len(fold_idx)} tamam ({time.time()-t0:.0f}s)')
print(f'LightGBM OOF AUC: {roc_auc_score(y, oof_lgb):.5f}  ({time.time()-t0:.0f}s)')

oof_xgb = np.zeros(len(Xv)); pred_xgb = np.zeros(len(Xtestv))
for fold, (tr, va) in enumerate(fold_idx):
    ...
    print(f'  XGBoost fold {fold+1}/{len(fold_idx)} tamam ({time.time()-t0:.0f}s)')
print(f'XGBoost OOF AUC: {roc_auc_score(y, oof_xgb):.5f}  ({time.time()-t0:.0f}s)')

oof_cat = np.zeros(len(Xv)); pred_cat = np.zeros(len(Xtestv))
for fold, (tr, va) in enumerate(fold_idx):
    ...
    m.fit(Xv[tr], y[tr], eval_set=Pool(Xv[va], y[va]), early_stopping_rounds=100)
    oof_cat[va] = m.predict_proba(Xv[va])[:, 1]
    pred_cat += m.predict_proba(Xtestv)[:, 1] / len(fold_idx)
    print(f'  CatBoost fold {fold+1}/{len(fold_idx)} tamam ({time.time()-t0:.0f}s)')
...
print(f'\n3-model blend (ORIG-eklenmis) OOF AUC = {auc:.5f}  (K1+A+B+D referans OOF: 0.96879, LB: 0.96985)')
...
print(f'\nORIG feature\'larinin importance siralamasi ({len(orig_ranks)} adet):')
print(orig_ranks[['rank', 'feature', 'gain_importance']].to_string(index=False))
print(f'\nEn iyi ORIG feature rank: {orig_ranks["rank"].min()} / {len(feature_names)}')
print(f'ORIG feature\'larinin medyan rank\'i: {orig_ranks["rank"].median():.0f} / {len(feature_names)}')
print(f'Top-20 icinde kac ORIG feature var: {(orig_ranks["rank"] <= 20).sum()}')

print('\nTum feature importance top 20:')
print(imp_df.head(20)[['rank', 'feature', 'gain_importance']].to_string(index=False))
