"""
Izole test: feature-selection fork'unun onerisi - A+B+D 84-ozellikli referanstan (LightGBM
solo OOF=0.96856, 3-model blend OOF=0.96879, LB=0.96985) LightGBM gain'i EN DUSUK 16
ozelligi cikar (68 kaldi). CV'de anlamli fark beklenmiyor (fork'un kendi testinde
84->52'de bile fark olcumez cikti) - amac CV kazanci degil, bugunku pair-lattice olayinin
(CV+iyi, LB-kotu, fazla karmasiklik) SIMETRIGINI test etmek: gereksiz ozellikleri cikarmak
CV'yi bozmadan LB'de varyans/overfit yuzeyini azaltip notr-ila-hafif-pozitif bir etki
yaratabilir mi?
Cikarilan 16 ozellik: 3 "olu" ham kategorik sutun (gender/stress_level/academic_work_impact,
to_numeric coerce bug'i yuzunden hep -999 sabit), bunlarin freq/TE kopyalarindan bir kismi,
age+imp_age, ve dusuk-kardinaliteli d1_* (ondalik basamak) sutunlarinin bir kismi.
"""
import pandas as pd
import numpy as np
import json
...
DROP_COLS = ['stress_level', 'academic_work_impact', 'gender', 'freq_academic_work_impact',
             'freq_gender', 'd1_social_media_hours', 'freq_stress_level', 'd1_gaming_hours',
             'd1_sleep_hours', 'd1_work_study_hours', 'd1_weekend_screen_time',
             'd1_daily_screen_time_hours', 'imp_age', 'age', 'te_academic_work_impact', 'te_gender']
...
X_df = pd.concat([raw_tr, enc_tr.add_prefix('te_'), freq_tr.add_prefix('freq_'), imp_tr], axis=1)
X_test_df = pd.concat([raw_te, enc_te.add_prefix('te_'), freq_te.add_prefix('freq_'), imp_te], axis=1)
print(f'Budama oncesi: {X_df.shape[1]} ozellik')
missing_drop = [c for c in DROP_COLS if c not in X_df.columns]
if missing_drop:
    raise ValueError(f'DROP_COLS icinde bulunamayan sutun(lar): {missing_drop}')
X_df = X_df.drop(columns=DROP_COLS)
X_test_df = X_test_df.drop(columns=DROP_COLS)
print(f'Budama sonrasi: {X_df.shape[1]} ozellik ({len(DROP_COLS)} cikarildi)')
...
print(f'LightGBM OOF AUC: {roc_auc_score(y, oof_lgb):.5f} ({time.time()-t0:.0f}s)  (ABD84 referans: 0.96856)')
...
print(f'XGBoost OOF AUC: {roc_auc_score(y, oof_xgb):.5f} ({time.time()-t0:.0f}s)  (ABD84 referans: 0.96776)')
...
print(f'CatBoost OOF AUC: {roc_auc_score(y, oof_cat):.5f} ({time.time()-t0:.0f}s)  (ABD84 referans: 0.96834)')

blend_oof = (rankdata(oof_lgb) + rankdata(oof_xgb) + rankdata(oof_cat)) / 3
print(f'\n3-model blend OOF AUC: {roc_auc_score(y, blend_oof):.5f}  (ABD84 referans: 0.96879, LB=0.96985)')
