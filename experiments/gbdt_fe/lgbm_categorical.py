...
# Sürekli sütunlar -> kategorik string (NaN da kendi kategorisi olur)
...
for frame in (train, test):
    for c in cont_cols:
        frame[c] = frame[c].astype('object').where(frame[c].notna(), 'missing')
...
print(f'\nCV OOF AUC (categorical): {cv_auc:.5f}')
