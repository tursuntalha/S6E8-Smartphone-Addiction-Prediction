"""
Izole test A+B+D: K1 bundle + frekans encoding (A, LB+0.00013 dogrulandi) + imputation-
augment (B, LB+0.00002 gurultu-seviyesi ama pozitif) + decimal lattice (D, LB+0.00001
gurultu-seviyesi ama pozitif). E (jenerator-region, LB-0.00006) ve C (catboost-native,
LB-0.00074) DISLANDI - ikisi de net negatifti.
Hipotez: B ve D'nin tek-submission cozunurlugumuzun altinda kalan kucuk gercek pozitif
etkileri olabilir, farkli bilgi kanallari tasidiklari icin (frekans/imputation/ondalik
birbirinden bagimsiz) toplaninca olculebilir hale gelebilir.
Tek-seed (42), 3-model rank-blend, A referansiyla (LB 0.96970) karsilastirilacak.
"""
import pandas as pd
import numpy as np
import json
...
X = pd.concat([raw_tr, enc_tr.add_prefix('te_'), freq_tr.add_prefix('freq_'), imp_tr], axis=1).values
X_test = pd.concat([raw_te, enc_te.add_prefix('te_'), freq_te.add_prefix('freq_'), imp_te], axis=1).values
print(f'Ozellik sayisi: {X.shape[1]} (K1 48 + {freq_tr.shape[1]} frekans + {imp_tr.shape[1]} imputed + {len(decimal_feat_cols)} decimal)')
...
print(f'\n3-model blend OOF AUC: {roc_auc_score(y, blend_oof):.5f}  (K1 referans: 0.96843, A-solo referans: 0.96868)')
...
sub_path = f'{SUB}/2026-08-19/lgbm_xgb_cat_ABD_combined_2026-08-19.csv'
sub.to_csv(sub_path, index=False)
print(f'Saved: {sub_path}')

np.save(f'{NN_CACHE}/gbdt_abd_oof.npy', blend_oof)
np.save(f'{NN_CACHE}/gbdt_abd_test_pred.npy', blend_pred)
print('Saved: nn_cache/gbdt_abd_oof.npy, nn_cache/gbdt_abd_test_pred.npy')
