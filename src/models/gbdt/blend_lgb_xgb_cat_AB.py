...
for frame in (train, test):
    ...
    # A: ek oranlar (LB dogrulandi, +0.00055)
    ...
    # B: farklar (LB dogrulandi, +0.00044)
...
print(f'Features: {X.shape[1]} (28 baseline + {len(extra_cols)} A+B) | enc: {time.time()-t0:.0f}s')
...
print(f'Karşılaştırma -> v4 blend (sadece 7 ratio, LB 0.96900): 0.96793 | v5 blend (+A+B): {blend_auc:.5f}')

sub = pd.DataFrame({'id': test['id'], 'addicted_label': pred_rank / max(pred_rank)})
sub_path = f'{SUB}/lgbm_xgb_cat_rankblend_v5_AB_2026-08-12.csv'
