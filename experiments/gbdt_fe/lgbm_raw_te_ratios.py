...
for frame in (train, test):
    ...
    # YENI RATIO OZELLIKLERI (2026-08-12) - paydalar hicbir zaman 0 olmuyor
    # (sleep_hours min=4.50, daily_screen_time_hours min=0.50), sifir bolme riski yok.
...
all_cats = cont_cols + cat_cols  # TE bu sete uygulanacak (ratio'lar TE'lenmiyor, zaten surekli)
...
print(f'Özellik sayısı: {Xc.shape[1]} (baseline 28 + {len(ratio_cols)} ratio) | encoding: {time.time()-t0:.0f}s')
...
print(f'\nCV OOF AUC (raw+TE+ratios): {cv_auc:.5f}')
print(f'Karşılaştırma -> baseline LGB (v3, SMOOTH=3): 0.96738 | +ratios: {cv_auc:.5f}')
imp = pd.Series(model.feature_importances_, index=Xc.columns).sort_values(ascending=False)
print('\nRatio özelliklerinin önem sırası:')
print(imp[ratio_cols].sort_values(ascending=False).to_string())
