"""
blend_multiseed.py'nin K1-bundle versiyonu: 42-ozellik AB seti yerine 48-ozellik K1
(AB - diff_daily_sum + diff_daily_sum_clean + Kategori I dominant-activity/rank).
K1 izole tek-LGB testte K0'a gore +0.00009 CV, LB'de +0.00006 (yeni en iyi tek-model
LB skoru, 0.96940) vermisti - burada ayni bagini 3-model x 3-seed bagging'e tasiyoruz.
"""
import pandas as pd
import numpy as np
import json
...
all_cats = cont_cols + cat_cols          # target-encoded + kept raw
te_only_cats = ['dominant_activity']     # sadece target-encoded (raw string, modele girmiyor)
...
all_pred = {}  # (model, seed) -> test pred array
...
ref_oof = (rankdata(all_oof[('lgb', 42)]) + rankdata(all_oof[('xgb', 42)]) + rankdata(all_oof[('cat', 42)])) / 3
ref_auc = roc_auc_score(y, ref_oof)
print(f'\nTek-seed (42) 3-model blend OOF AUC: {ref_auc:.5f}  (referans: K0/AB multiseed blend 0.96869)')
...
print(f'Cok-seedli (3 seed ort, model-bazli sonra blend) OOF AUC: {multiseed_auc:.5f}  (delta vs tek-seed={multiseed_auc-ref_auc:+.5f})')
print(f'  (referans: K0/AB multiseed blend 0.96869, delta vs K0 multiseed={multiseed_auc-0.96869:+.5f})')

all_ranks_oof = sum(rankdata(all_oof[k]) for k in all_oof) / len(all_oof)
allrank_auc = roc_auc_score(y, all_ranks_oof)
print(f'Tum 9 (model x seed) dogrudan rank-blend OOF AUC: {allrank_auc:.5f}  (delta={allrank_auc-ref_auc:+.5f})')
...
sub_path = f'{SUB}/lgbm_xgb_cat_multiseed_K1_2026-08-14.csv'
