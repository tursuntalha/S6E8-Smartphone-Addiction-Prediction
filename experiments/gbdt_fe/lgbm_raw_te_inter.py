...
# ETKİLEŞİM: güçlü TE özelliklerinin çarpımları
inter_feats = ['daily_screen_time_hours', 'social_media_hours', 'gaming_hours',
               'weekend_screen_time', 'notifications_per_day', 'app_opens_per_day', 'work_study_hours']
inter_tr = pd.DataFrame(index=train.index)
inter_te = pd.DataFrame(index=test.index)
for i in range(len(inter_feats)):
    for j in range(i + 1, len(inter_feats)):
        a, b = inter_feats[i], inter_feats[j]
        inter_tr[f'{a}_x_{b}'] = enc_tr[a] * enc_tr[b]
        inter_te[f'{a}_x_{b}'] = enc_te[a] * enc_te[b]

Xc = pd.concat([raw_tr, enc_tr.add_prefix('te_'), inter_tr.add_prefix('ix_')], axis=1)
Xc_test = pd.concat([raw_te, enc_te.add_prefix('te_'), inter_te.add_prefix('ix_')], axis=1)
print(f'Özellik sayısı: {Xc.shape[1]} (raw 14 + TE 14 + inter {inter_tr.shape[1]}) | enc {time.time()-t0:.0f}s')
...
print(f'\nCV OOF AUC (raw+TE+inter): {roc_auc_score(y, oof):.5f}')
